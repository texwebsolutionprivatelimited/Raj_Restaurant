...
export default function Inventory() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/api/admin/menu", {
      ...
      .then((data) => {
        ...
        setItems(safeItems);// IMPORTANT FIX
      })
  ...
  const handleSaveItem = async () => {
    ...
    formData.append("category", newItem.category); // ONLY 4 allowed
    // formData.append("isVeg", newItem.type === "Veg");     // REQUIRED
    ...
    try {
      ...
      if (editId) {
        res = await fetch(
          `http://localhost:3000/api/admin/menu/${editId}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        res = await fetch("http://localhost:3000/api/admin/menu", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      // ✅ FIX: backend returns direct object
      const item = data;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(
        `http://localhost:3000/api/admin/menu/${id}`,
        {
          method: "DELETE",
        }
      );
